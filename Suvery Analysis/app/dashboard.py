...
def display_dashboard(df, insights):
    ...
    st.write("### Survey and Quiz Responses")
    st.dataframe(filtered_df)

    st.write("### Sentiment vs Score Relationship")
    fig, ax = plt.subplots()
    ...
    st.write("### Average Score by Region")
    ...
    st.write("### Gender-Based Averages")
    ...
    from app.insights import generate_insights, generate_temporal_insights, generate_weekly_trend_insight
    filtered_insights = generate_insights(filtered_df) + generate_temporal_insights(filtered_df) + generate_weekly_trend_insight(filtered_df)
